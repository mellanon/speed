...
def speedtest(request):
    ...
    cmd = ['/usr/bin/python', '-u', path+'speedtest_cli.py']
	...
	rqStatus = request['request']['rqstatus']
        ...
        if srvList != None and len(srvList) > 0 and not speedTestMiniEnabled:
            cmd = ['/usr/bin/python', '-u', path+'speedtest_cli.py', '--server', srvList[0]]
            ...
            cmd = ['/usr/bin/python', '-u', path+'speedtest_cli.py', '--mini', speedTestMiniServer]
